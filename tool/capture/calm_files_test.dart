    ...
    for (final scale in [1.0, 2.0]) {
      for (final empty in [false, true]) {
        testWidgets('Files ${light ? 'light' : 'dark'} $scale empty=$empty', (
          ...
          await writePng(
            'docs/qa/calm-files/$_captureSet/files-$tone-${scale.toInt()}x-${empty ? 'empty' : 'populated'}.png',
            await capturePng(tester, boundary),
          );
          if (!empty) {
            await tester.tap(
              find.byKey(const ValueKey('file-surface-selector')),
            );
            await tester.pumpAndSettle();
            expect(find.text('Symbols'), findsOneWidget);
            expect(tester.takeException(), isNull);
            await writePng(
              'docs/qa/calm-files/$_captureSet/files-$tone-${scale.toInt()}x-search-options.png',
              await capturePng(tester, boundary),
            );
            await tester.tap(
              find.ancestor(
                of: find.text('Files').last,
            ...
            await tester.pumpAndSettle();
            expect(find.text('6 files · +938 −0'), findsOneWidget);
            expect(tester.takeException(), isNull);
            await writePng(
              'docs/qa/calm-files/$_captureSet/files-$tone-${scale.toInt()}x-changes.png',
              await capturePng(tester, boundary),
            );
            await tester.tapAt(const Offset(8, 70));
            await tester.pumpAndSettle();
            ...
            expect(tester.takeException(), isNull);
            await writePng(
              'docs/qa/calm-files/$_captureSet/files-$tone-${scale.toInt()}x-folder.png',
              await capturePng(tester, boundary),
            );
          }
