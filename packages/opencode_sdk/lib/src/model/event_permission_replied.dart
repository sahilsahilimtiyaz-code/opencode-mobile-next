//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//

// ignore_for_file: unused_element
import 'package:opencode_sdk/src/model/permission_replied_data.dart';
import 'package:json_annotation/json_annotation.dart';
import 'package:equatable/src/equatable_utils.dart';

part 'event_permission_replied.g.dart';

@JsonSerializable(
  checked: true,
  createToJson: true,
  disallowUnrecognizedKeys: false,
  explicitToJson: true,
)
class EventPermissionReplied {
  /// Returns a new [EventPermissionReplied] instance.
  EventPermissionReplied({
    required this.id,

    required this.type,

    required this.properties,
  });

  @JsonKey(name: r'id', required: true, includeIfNull: false)
  final String id;

  @JsonKey(
    name: r'type',
    required: true,
    includeIfNull: false,
    unknownEnumValue: EventPermissionRepliedTypeEnum.unknownDefaultOpenApi,
  )
  final EventPermissionRepliedTypeEnum type;

  @JsonKey(name: r'properties', required: true, includeIfNull: false)
  final PermissionRepliedData properties;

  bool operator ==(Object other) {
    return identical(this, other) ||
        other is EventPermissionReplied &&
            runtimeType == other.runtimeType &&
            equals(
              [id, type, properties],
              [other.id, other.type, other.properties],
            );
  }

  int get hashCode =>
      runtimeType.hashCode ^ mapPropsToHashCode([id, type, properties]);

  factory EventPermissionReplied.fromJson(Map<String, dynamic> json) =>
      _$EventPermissionRepliedFromJson(json);

  Map<String, dynamic> toJson() => _$EventPermissionRepliedToJson(this);

  String toString() {
    return toJson().toString();
  }
}

enum EventPermissionRepliedTypeEnum {
  @JsonValue(r'permission.replied')
  permissionPeriodReplied(r'permission.replied'),
  @JsonValue(r'unknown_default_open_api')
  unknownDefaultOpenApi(r'unknown_default_open_api');

  const EventPermissionRepliedTypeEnum(this.value);

  final Object value;

  @override
  String toString() => value.toString();
}
